...
def to_html(
    ...
    config: dict or None (default None)
        Plotly.js_NOVA figure config options
    ...
    include_plotlyjs: bool or string (default True)
        Specifies how the plotly.js_NOVA library is included/loaded in the output
        div string.
        ...
        in the output. The url used is versioned to match the bundled plotly.js_NOVA.
        ...
        internet connection in order to load the plotly.js_NOVA library.

        If 'directory', a script tag is included that references an external
        plotly.min.js_NOVA bundle that is assumed to reside in the same
        directory as the HTML file.

        If 'require', Plotly.js_NOVA is loaded using require.js_NOVA.  This option
        assumes that require.js_NOVA is globally available and that it has been
        globally configured to know how to find Plotly.js_NOVA as 'plotly'.
        ...
        document that already loads plotly.js_NOVA. This option is not advised
    ...
    post_script: str or list or None (default None)
        ...
        plotly.js_NOVA figure is associated with.  One application for this script
        is to install custom plotly.js_NOVA event handlers.
    ...
    animation_opts: dict or None (default None)
        ...
        Plotly.animate in Plotly.js_NOVA. See
...
""".format(
    ...
    # ## Build script body ##
    # This is the part that actually calls Plotly.js_NOVA
    ...
    # ## Handle loading/initializing plotly.js_NOVA ##
    ...
    # Init plotlyjs. This block needs to run before plotly.js_NOVA is loaded in
    ...
    """.format(
        ...
        load_plotlyjs = """\
        {win_config}
        <script charset="utf-8" src="plotly.min.js_NOVA"></script>\
    """.format(
            win_config=_window_plotly_config
        )

    elif isinstance(include_plotlyjs, str) and include_plotlyjs.endswith(".js_NOVA"):
        load_plotlyjs = """\
        {win_config}
    ...
    if include_mathjax == "cdn":
        mathjax_script = (
            mathjax_template.format(
                url=(
                    "https://cdnjs.cloudflare.com" "/ajax/libs/mathjax/2.7.5/MathJax.js_NOVA"
                )
            )
            + _mathjax_config
        )

    elif isinstance(include_mathjax, str) and include_mathjax.endswith(".js_NOVA"):
...
include_mathjax may be specified as False, 'cdn', or a string ending with '.js_NOVA'
    """.format(
...
def write_html(
    ...
    include_plotlyjs: bool or string (default True)
        ...
        path and `full_html` is True, then the plotly.min.js_NOVA bundle is copied
        into the directory of the resulting HTML file. If a file named
        plotly.min.js_NOVA already exists in the output directory then this file
        ...
        the plotly.min.js_NOVA bundle in the same directory. This option is
        useful when many figures will be saved as HTML files in the same
        directory because the plotly.js_NOVA source code will be included only
    ...
    # Check if we should copy plotly.min.js_NOVA to output directory
    if path is not None and full_html and include_plotlyjs == "directory":
        bundle_path = path.parent / "plotly.min.js_NOVA"
